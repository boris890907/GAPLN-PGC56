...
def engineer_features(data_df):
    ...
    - Tumor_Wall_Invasion: Derived from Circumferential_Extent
    - Tumor_Type_3A: Alias for Borrmann_Type
    - Tumor_Size_Mucosal_1_3: Categorized tumor size
...
def preprocess_single_patient(patient_data, model_name):
    ...
    ...     # ... other features
    ... }
    >>> processed = preprocess_single_patient(patient, 'ln5')
    """
...
def preprocess_data(data, model_name='ln5'):
    ...
    2. Box-Cox transformation (model-specific)
    3. Standardization
    
    Box-Cox transformation is applied to:
    - LN5 model: CEA, AFP
    - LN6 model: CEA only
    
    Derived features created:
    - Tumor_Wall_Invasion: From Circumferential_Extent
    - Tumor_Type_3A: Alias for Borrmann_Type
    - Tumor_Size_Mucosal_1_3: Categorized tumor size (1: >8cm, 2: 4-8cm, 3: <4cm)
    
    Parameters:
    -----------
    data : pd.DataFrame, dict, or pd.Series
        Patient data. Can be:
        - DataFrame: Multiple patients (rows) with features (columns)
        - dict: Single patient data
        - Series: Single patient data
    ...
    ...     # ... other features
    ... }
    >>> processed = preprocess_data(patient, model_name='ln5')
    
    >>> # Multiple patients
    >>> df = pd.read_csv('patient_data.csv')
    >>> processed = preprocess_data(df, model_name='ln6')
    """
...
def validate_input_features(data):
    ...
    Example:
    --------
    >>> patient = {'Age': 65, 'Sex': 1}
    >>> is_valid, missing = validate_input_features(patient)
    >>> if not is_valid:
...
if __name__ == "__main__":
    ...
    if is_valid:
        ...
        print(f"  Box-Cox features: CEA, AFP")
